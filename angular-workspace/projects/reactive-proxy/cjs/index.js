...
exports.ReactiveState = exports.PrimitiveBox = void 0;
/**
 * Clase para envolver valores primitivos y dotarlos de metadatos _exe_
 */
class PrimitiveBox {
    constructor(value, _exe_) {
        this.value = value;
        this._exe_ = _exe_;
    }
...
class ReactiveState {
     ...
     */
    static unwrap(val) {
        if (val instanceof PrimitiveBox)
            return val.value;
    ...
    _createProxy(path, target, parent, key, type) {
        const self = this;
        ...
        proxyInstance = new Proxy(target, {
            get(target, prop, receiver) {
                ...
                if (prop === '_exe_') {
                    return {
                        ...
                        type: currentType,
                        cleanNode: target,
                        subscribe: (cb) => self._subscribe(path, cb),
                        getRoot: () => {
                        ...
                        toJS: () => {
                            if (isArray)
                                return target.map((i) => ReactiveState.unwrap(i));
                            if (isMap) {
                                const copy = new Map();
                                target.forEach((v, k) => copy.set(ReactiveState.unwrap(k), ReactiveState.unwrap(v)));
                                return copy;
                            }
                            if (isSet) {
                                const copy = new Set();
                                target.forEach((v) => copy.add(ReactiveState.unwrap(v)));
                                return copy;
                            }
                            const copy = {};
                ...
                if (value !== undefined && typeof prop !== 'symbol' && typeof value !== 'function' && prop !== '_exe_' && !PROTECTED_CORE_KEYS.has(String(prop))) {
                    ...
                    if (!isReserved) {
                        if (value !== null && typeof value === 'object' && !(value instanceof PrimitiveBox)) {
                            let childType = Array.isArray(value) ? 'array' : value instanceof Map ? 'map' : value instanceof Set ? 'set' : 'object';
            ...
            set(target, prop, value, receiver) {
                ...
                // Protecciones
                if (PROTECTED_CORE_KEYS.has(propStr))
                    throw new Error(`[ReactiveState] Error: '${propStr}' es de solo lectura.`);
                if (isMap && RESERVED_MAP_KEYS.has(propStr))
                    throw new Error(`[ReactiveState] Error: No puedes sobrescribir el método '${propStr}' en Map.`);
                if (isSet && RESERVED_SET_KEYS.has(propStr))
                    throw new Error(`[ReactiveState] Error: No puedes sobrescribir el método '${propStr}' en Set.`);
            ...
            deleteProperty(target, prop) {
                if (PROTECTED_CORE_KEYS.has(String(prop)))
                    throw new Error(`[ReactiveState] Error: No se puede eliminar '_exe_'.`);
    ...
    _assimilate(value, path, parent, key, type) {
        ...
        const primitiveExe = {
            ...
            cleanNode: value,
            toJS: () => value,
            subscribe: (cb) => this._subscribe(path, cb),
            getRoot: () => {
                let current = parent;
                while (current && current._exe_ && current._exe_.parent !== null)
                    current = current._exe_.parent;
                return current;
            }
        };
