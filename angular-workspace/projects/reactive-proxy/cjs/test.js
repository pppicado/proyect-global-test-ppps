...
async function runTests() {
    ...
    test("Estructura recursiva y tipos", () => {
        const store = new index_1.ReactiveState({ user: { profile: { name: "Aria" } } });
    ...
    test("Boxeo de primitivos (PrimitiveBox)", () => {
        const store = new index_1.ReactiveState({ contador: 10 });
    ...
    test("Creación automática de rutas (Objetos y Arrays)", () => {
        const store = new index_1.ReactiveState();
    ...
    test("Suscripciones granulares y burbujeo", () => {
        const store = new index_1.ReactiveState({ datos: { valor: 100 } });
        let notified = false;
    ...
    test("Maps: Notación de puntos y protección", () => {
        const store = new index_1.ReactiveState({ ajustes: new Map() });
    ...
    test("Sets: Transparencia de identidad", () => {
        const store = new index_1.ReactiveState({ etiquetas: new Set() });
    ...
    test("Navegación por el árbol (Parent/Root)", () => {
        const store = new index_1.ReactiveState({ a: { b: { c: 1 } } });
    ...
    test("Exportación limpia (toJS vs cleanNode)", () => {
        ...
        const store = new index_1.ReactiveState(dataInicial);
        ...
        console.assert(!(snapshot.user.name instanceof index_1.PrimitiveBox), "toJS debería desboxear");
    ...
    test("Protección de la propiedad _exe_", () => {
        const store = new index_1.ReactiveState({ a: 1 });
        try {
    ...
    test("Mutación directa de primitivos (+=)", () => {
        const store = new index_1.ReactiveState({ app: { nombre: "Mi App" } });
